...
public class PageAdmin extends JFrame {
    ...
    private void openArticleDialog(Article art) {
        ...
        JTextField fPrix= new JTextField(isNew?"":" "+art.getPrix());
        JTextField fStock=new JTextField(isNew?"":""+art.getStock());
        ...
        JButton ok = new JButton("OK"), cancel=new JButton("Annuler");
    ...
    private void initClientsTab() {
        modelCommandes = new DefaultTableModel(
                new String[]{"ID","User ID","Date","Total"},0)
        { @Override public boolean isCellEditable(int r,int c){return false;} };
        ...
        // Sur sélection d’une commande on recharge les détails
        tabCommandes.getSelectionModel()
                .addListSelectionListener(evt -> {
                    if (!evt.getValueIsAdjusting()) {
                        int r = tabCommandes.getSelectedRow();
                        if (r>=0) {
                            int id = (int) modelCommandes.getValueAt(r,0);
                            reloadDetails(id);
                        }
                    }
                });
